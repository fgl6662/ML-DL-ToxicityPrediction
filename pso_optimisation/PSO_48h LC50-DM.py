...
# 定义模型
my_knn_md = make_pipeline(MinMaxScaler(),VarianceThreshold(), KNeighborsClassifier(n_neighbors=7))
my_svm_md = make_pipeline(MinMaxScaler(),VarianceThreshold(), SVC(C=1,gamma=10, probability=True))
my_xgb_mol2vec = make_pipeline(VarianceThreshold(), XGBClassifier(random_state=42, n_estimators=50, max_depth=3, colsample_bytree=0.4, learning_rate=0.001))
...
train_md = np.load(f"../../data/featurised_data/{spec}/rdkit_md_train.npy", allow_pickle=True)
...
test_md = np.load(f"../../data/featurised_data/{spec}/rdkit_md_test.npy", allow_pickle=True)
...
val_md = np.load(f"../../data/featurised_data/{spec}/rdkit_md_val.npy", allow_pickle=True)
...
model_path = f"../../model_tuning/AttentiveFP/result/model/48h LC50-DM/gnn.pth"

model.load_state_dict(torch.load(model_path, map_location=device))

val_res, val_labels, masks = evaluate_model(model, test_loader, device)
val_pred_gnn_graph = val_res[masks == 1]
val_pred_knn_md = my_knn_md.fit(train_md, train_label).predict_proba(test_md)[:, 1]
val_pred_svm_md = my_svm_md.fit(train_md, train_label).predict_proba(test_md)[:, 1]
val_pred_xgb_mol2vec = my_xgb_mol2vec.fit(train_mol2vec, train_label).predict_proba(test_mol2vec)[:, 1]

val_pred_list = [val_pred_knn_md, val_pred_svm_md, val_pred_xgb_mol2vec, val_pred_gnn_graph]
auc_optimization_func = get_optimasation_function(val_pred_list, test_label)
...
# 使用各个模型进行预测
test_pred_knn_md = my_knn_md.fit(train_md, train_label).predict_proba(test_md)[:, 1]
test_pred_svm_md = my_svm_md.fit(train_md, train_label).predict_proba(test_md)[:, 1]
...
# 计算各个模型的 AUC-ROC 分数
test_roc_auc_knn_md = roc_auc_score(test_label, test_pred_knn_md)
test_roc_auc_svm_md = roc_auc_score(test_label, test_pred_svm_md)
...
# 合并所有模型的预测结果
test_pred_top4ensemble = (test_pred_knn_md * w1_avage_norm + test_pred_svm_md * w2_avage_norm + test_pred_xgb_mol2vec * w3_avage_norm + test_pred_gnn_graph * w4_avage_norm)

# 计算集成模型的 AUC-ROC 分数
test_roc_auc_top4ensemble = roc_auc_score(test_label, test_pred_top4ensemble)

# 导出结果
test_roc_list = [test_roc_auc_knn_md, test_roc_auc_svm_md, test_roc_auc_xgb_mol2vec, test_roc_auc_gnn_graph, test_roc_auc_top4ensemble]
weight_list = [w1_avage_norm, w2_avage_norm, w3_avage_norm, w4_avage_norm, 1]
fea_list = ['knn_md', 'svm_md', 'xgb_mol2vec', 'gnn_graph', 'top4ensemble']
weight_path = f"../../PSOresults/48h LC50-DM/weights"
if not os.path.isdir(weight_path):
    os.makedirs(weight_path)


pred_path = f"../../PSOresults/48h LC50-DM/pred/top4"
...
models = ['knn_md', 'svm_md', 'xgb_mol2vec', 'gnn_graph', 'top4ensemble']
preds = [test_pred_knn_md, test_pred_svm_md, test_pred_xgb_mol2vec, test_pred_gnn_graph, test_pred_top4ensemble]
auc_scores = [test_roc_auc_knn_md, test_roc_auc_svm_md, test_roc_auc_xgb_mol2vec, test_roc_auc_gnn_graph, test_roc_auc_top4ensemble]
...
plt.title('ROC-AUC curves of XRA-ens and its base classifiers on the 48h LC50-DM Dataset')
