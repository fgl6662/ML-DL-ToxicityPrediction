...
# 定义模型
my_svm_mol2vec = make_pipeline(VarianceThreshold(threshold), SVC(random_state=42,C=10,gamma=0.001, probability=True))
my_xgb_mol2vec = make_pipeline(VarianceThreshold(threshold), XGBClassifier(random_state=42, n_estimators=50, max_depth=5, colsample_bytree=0.3, learning_rate=0.1))
my_xgb_mf1024 = make_pipeline(VarianceThreshold(threshold), XGBClassifier(random_state=42, n_estimators=50, max_depth=7, colsample_bytree=0.3, learning_rate=0.1))
...
model_path = f"../../model_tuning/AttentiveFP/result/model/40h IGC50/gnn.pth"
...
test_pred_list = [test_pred_svm_mol2vec, test_pred_xgb_mol2vec, test_pred_xgb_mf1024, test_pred_gnn_graph]
...
# 合并所有模型的预测结果
test_pred_top4ensemble = (test_pred_svm_mol2vec * w1_avage_norm + test_pred_xgb_mol2vec * w2_avage_norm + test_pred_xgb_mf1024 * w3_avage_norm + test_pred_gnn_graph * w4_avage_norm)

# 计算集成模型的 AUC-ROC 分数
test_roc_auc_top4ensemble = roc_auc_score(test_label, test_pred_top4ensemble)

# 导出结果
test_roc_list = [test_roc_auc_svm_mol2vec, test_roc_auc_xgb_mol2vec, test_roc_auc_xgb_mf1024, test_roc_auc_gnn_graph, test_roc_auc_top4ensemble]
weight_list = [w1_avage_norm, w2_avage_norm, w3_avage_norm, w4_avage_norm, 1]
fea_list = ['svm_mol2vec', 'xgb_mol2vec', 'xgb_mf1024', 'gnn_graph', 'top4ensemble']
weight_path = f"../../PSOresults/40h IGC50/weights"
if not os.path.isdir(weight_path):
    os.makedirs(weight_path)


pred_path = f"../../PSOresults/40h IGC50/pred/top4"
...
models = ['svm_mol2vec', 'xgb_mol2vec', 'xgb_mf1024', 'gnn_graph', 'top4ensemble']
preds = [test_pred_svm_mol2vec, test_pred_xgb_mol2vec, test_pred_xgb_mf1024, test_pred_gnn_graph, test_pred_top4ensemble]
auc_scores = [test_roc_auc_svm_mol2vec, test_roc_auc_xgb_mol2vec, test_roc_auc_xgb_mf1024, test_roc_auc_gnn_graph, test_roc_auc_top4ensemble]
...
plt.title('ROC-AUC curves of XRA-ens and its base classifiers on the 40h IGC Dataset')
