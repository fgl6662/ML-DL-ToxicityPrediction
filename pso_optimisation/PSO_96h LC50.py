...
# 定义模型
my_xgb_mf2048 = make_pipeline(VarianceThreshold(threshold), XGBClassifier(random_state=42, n_estimators=50, max_depth=8, colsample_bytree=0.3, learning_rate=0.1))
my_xgb_mf1024 = make_pipeline(VarianceThreshold(threshold), XGBClassifier(random_state=42, n_estimators=50, max_depth=8, colsample_bytree=0.2, learning_rate=0.1))
my_rf_mf2048 = make_pipeline(VarianceThreshold(threshold), RandomForestClassifier(random_state=42, n_estimators=50, max_depth=8, max_features=0.5, min_samples_split=4))
...
train_mf1024 = np.load(f"../../data/featurised_data/{spec}/fp1024_train.npy", allow_pickle=True)
train_mf2048 = np.load(f"../../data/featurised_data/{spec}/fp2048_train.npy", allow_pickle=True)
train_label = np.load(f"../../data/featurised_data/{spec}/label_train.npy", allow_pickle=True)

val_mf1024 = np.load(f"../../data/featurised_data/{spec}/fp1024_val.npy", allow_pickle=True)
val_mf2048 = np.load(f"../../data/featurised_data/{spec}/fp2048_val.npy", allow_pickle=True)
val_label = np.load(f"../../data/featurised_data/{spec}/label_val.npy", allow_pickle=True)

test_mf1024 = np.load(f"../../data/featurised_data/{spec}/fp1024_test.npy", allow_pickle=True)
test_mf2048 = np.load(f"../../data/featurised_data/{spec}/fp2048_test.npy", allow_pickle=True)
...
model_path = f"../../model_tuning/AttentiveFP/result/model/96h LC50/gnn.pth"
model.load_state_dict(torch.load(model_path, map_location=device))

test_res, test_labels, masks = evaluate_model(model, test_loader, device)
test_pred_gnn_graph = test_res[masks == 1]
test_pred_xgb_mf2048 = my_xgb_mf2048.fit(train_mf2048, train_label).predict_proba(test_mf2048)[:, 1]
test_pred_xgb_mf1024 = my_xgb_mf1024.fit(train_mf1024, train_label).predict_proba(test_mf1024)[:, 1]
test_pred_rf_mf2048 = my_rf_mf2048.fit(train_mf2048, train_label).predict_proba(test_mf2048)[:, 1]

test_pred_list = [test_pred_xgb_mf2048, test_pred_xgb_mf1024, test_pred_rf_mf2048, test_pred_gnn_graph]
...
# 计算各个模型的 AUC-ROC 分数
test_roc_auc_xgb_mf2048 = roc_auc_score(test_label, test_pred_xgb_mf2048)
test_roc_auc_xgb_mf1024 = roc_auc_score(test_label, test_pred_xgb_mf1024)
test_roc_auc_rf_mf2048 = roc_auc_score(test_label, test_pred_rf_mf2048)
test_roc_auc_gnn_graph = roc_auc_score(test_label, test_pred_gnn_graph)

# 合并所有模型的预测结果
test_pred_top4ensemble = (test_pred_xgb_mf2048 * w1_avage_norm + test_pred_xgb_mf1024 * w2_avage_norm + test_pred_rf_mf2048 * w3_avage_norm + test_pred_gnn_graph * w4_avage_norm)

# 计算集成模型的 AUC-ROC 分数
test_roc_auc_top4ensemble = roc_auc_score(test_label, test_pred_top4ensemble)

# 导出结果
test_roc_list = [test_roc_auc_xgb_mf2048, test_roc_auc_xgb_mf1024, test_roc_auc_rf_mf2048, test_roc_auc_gnn_graph, test_roc_auc_top4ensemble]
weight_list = [w1_avage_norm, w2_avage_norm, w3_avage_norm, w4_avage_norm, 1]
fea_list = ['xgb_mf2048', 'xgb_mf1024', 'rf_mf2048', 'gnn_graph', 'top4ensemble']
weight_path = f"../../PSOresults/96h LC50/weights"
if not os.path.isdir(weight_path):
    os.makedirs(weight_path)


pred_path = f"../../PSOresults/96h LC50/pred/top4"
...
models = ['xgb_mf2048', 'xgb_mf1024', 'rf_mf2048', 'gnn_graph', 'top4ensemble']
preds = [test_pred_xgb_mf2048, test_pred_xgb_mf1024, test_pred_rf_mf2048, test_pred_gnn_graph, test_pred_top4ensemble]
auc_scores = [test_roc_auc_xgb_mf2048, test_roc_auc_xgb_mf1024, test_roc_auc_rf_mf2048, test_roc_auc_gnn_graph, test_roc_auc_top4ensemble]
...
plt.title('ROC-AUC curves of XRA-ens and its base classifiers on the 96h LC50 Dataset ')
