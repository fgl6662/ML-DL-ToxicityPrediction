...
X_data = np.load('../../../data/featurised_data/{}/rdkit_md_train.npy'.format(spec), allow_pickle=True)
y_data = np.load('../../../data/featurised_data/{}/label_train.npy'.format(spec))
test_train = np.load(f'../../../data/featurised_data/{spec}/rdkit_md_test.npy', allow_pickle=True)
...
start_knn = time.time()
knn_tuning(X_data, y_data,test_train,test_label, tag="md", para=knn_para)
end_knn = time.time()
print("KNN Time:{} seconds".format(end_knn-start_knn))


start_svm = time.time()
svm_tuning(X_data, y_data,test_train,test_label, tag="md", para=svm_para)
end_svm = time.time()
print("SVM Time:{} seconds".format(end_svm-start_svm))


start_xgb = time.time()
xgb_tuning(X_data, y_data,test_train,test_label, tag="md", para=xgb_para)
end_xgb = time.time()
print("XGB Time:{} seconds".format(end_xgb-start_xgb))


start_rf = time.time()
rf_tuning(X_data, y_data,test_train,test_label, tag="md", para=rf_para)
end_rf = time.time()
